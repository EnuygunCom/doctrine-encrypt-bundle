...
namespace EnuygunCom\DoctrineEncryptBundle\Subscribers;
...
use \ReflectionClass;
use EnuygunCom\DoctrineEncryptBundle\Encryptors\EncryptorInterface;
...
class DoctrineEncryptSubscriber implements EventSubscriber {
    ...
    const ENCRYPTOR_INTERFACE_NS = 'EnuygunCom\DoctrineEncryptBundle\Encryptors\EncryptorInterface';

    /**
     * Encrypted annotation full name
     */
    const ENCRYPTED_ANN_NAME = 'EnuygunCom\DoctrineEncryptBundle\Configuration\Encrypted';
        ...
        foreach ($properties as $refProperty) {
            ...
            if($this->annReader->getPropertyAnnotation($refProperty, 'Doctrine\ORM\Mapping\ManyToOne')) {
                if ($reflectionClass->hasMethod($getter = 'get' . $methodName) && $reflectionClass->hasMethod($setter = 'set' . $methodName)) {
                    $_entity = $entity->$getter();
                    if(!empty($_entity)) {
                        $this->processFields($_entity, $isEncryptOperation);
                    }
                }
            }
